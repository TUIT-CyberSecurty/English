...
function submitStudentInfo() {
    ...
    if (name && surname) {
        addStudentToRanking(name, surname);
        closeForm();
    } else {
...
function addStudentToRanking(firstName, lastName) {
    ...
    // Проверяем и убираем класс hidden или меняем display
    if (ratingSection.classList.contains("hidden")) {
        ratingSection.classList.remove("hidden");
    } else if (ratingSection.style.display === "none" || getComputedStyle(ratingSection).display === "none") {
